...
class ActNorm(nn.Module):
    ...
    def forward(self, input):
        ...
        scale = self.scale #+ torch.sign(self.scale) * 1e-6
        log_abs = logabs(scale)

        logdet = height * width * torch.sum(log_abs)

        if self.logdet:
            return scale * (input + self.loc), logdet

        else:
            return scale * (input + self.loc)

    def reverse(self, output):
        #return output / (self.scale + torch.sign(self.scale) * 1e-6) - self.loc
...
class InvConv2d(nn.Module):
    ...
    def forward(self, input):
        ...
        if not np.isfinite(logdet.data.cpu().numpy()):
            print("Encountered non-finite value in ActNorm logdet")
...
class InvConv2dLU(nn.Module):
    ...
    def forward(self, input):
        ...
        if not np.isfinite(logdet.data.cpu().numpy()):
            print("Encountered non-finite value in 1x1 Conv logdet")
...
class AffineCoupling(nn.Module):
    def __init__(self, in_channel, filter_size=512, affine=True):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(filter_size, filter_size, 1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv2d(filter_size, in_channel if self.affine else in_channel // 2, 1),
        )
    ...
    def forward(self, input):
        ...
        if self.affine:
            ...
            log_s = (2/np.pi)*torch.atan(log_s)
            s = torch.exp(log_s)
            #s = F.softplus(log_s)*RLOG2
            # out_a = s * in_a + t
            out_b = (in_b + t) * s
...
class CondAffineCoupling(nn.Module):
    def __init__(self, in_channel, filter_size=512, affine=True, n_cond=512):
        super().__init__()
    ...
    def forward(self, input, cond):
        ...
        if self.affine:
            log_s, t = self.net(torch.cat((in_a, cond), 1)).chunk(2, 1)
            log_s = (2/np.pi)*torch.atan(log_s)
            s = torch.exp(log_s)
            #s = F.softplus(log_s)*RLOG2
            # out_a = s * in_a + t
            ...
            net_out = self.net(torch.cat((in_a, cond), 1))
            out_b = in_b + net_out
            logdet = None
    ...
    def reverse(self, output, cond):
        ...
        if self.affine:
            log_s, t = self.net(torch.cat((out_a, cond), 1)).chunk(2, 1)
            log_s = (2/np.pi)*torch.atan(log_s)
            s = torch.exp(log_s)
            #s = F.softplus(log_s)*RLOG2
            # in_a = (out_a - t) / s
            ...
            net_out = self.net(torch.cat((out_a, cond), 1))
            in_b = out_b - net_out
...
class CondFlow(nn.Module):
    def __init__(self, in_channel, affine=True, conv_lu=True, n_cond=512):
        ...
        self.coupling = CondAffineCoupling(in_channel, n_cond=n_cond, affine=affine)
...
class Block(nn.Module):
    def __init__(self, in_channel, n_flow, split=True, affine=True, conv_lu=True):
        ...
        if split:
            self.prior = Prior(in_channel * 2, in_channel * 4)

        else:
            self.prior = Prior(in_channel * 4, in_channel * 8)
